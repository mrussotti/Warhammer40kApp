...
const CreateArmy = ({ navigation }) => {
    ...
    const handleSquadPress = (squad) => {
        // Save the current state of the army before navigating
        const armyData = {
            name,
            faction,
        ...
        armyRef.set(armyData).then(() => {
            // Navigate to SquadCustomization and pass both the squad and the armyRef
            // so the squad customization can be saved directly to Firestore
            navigation.navigate('SquadCustomization', {
                squad: squad,
                armyId: armyId,
                setArmyUnits: setArmyUnits, // Added
                armyUnits: armyUnits // Added
            });
        });
    }
